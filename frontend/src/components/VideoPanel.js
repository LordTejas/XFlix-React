...
export default function VideoPanel({ videos }) {
  const VideoCard = ({
    ...
    return (
      ...
      <Link href={`/video/${id}`} className="video-tile-link">
        <Box
          sx={{
            color: "common.white",
          }}
          className="video-card video-tile"
        >
          <img
            // sx={{ height: 140 }}
            src={previewImage}
          ...
          <Typography variant="h6" className="video-card-title">
            {" "}
            {title}{" "}
          </Typography>

          <Typography variant="caption" className="video-card-timestamp">
            {" "}
  ...
  const videoTab = (video) => {
    return (
      <Grid item xs={6} md={4} lg={3} className="video-tab" key={video.id}>
        {VideoCard(video)}
      </Grid>
    );
  };
